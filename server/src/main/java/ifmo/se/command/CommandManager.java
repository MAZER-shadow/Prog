...
public class CommandManager {
     ...
     */
    public AbstractResponse execute(String input, AbstractRequest request) {
        try {
            ...
            if (parameters.length == SUM_WORDS_OF_COMMAND_NAME_AND_ARGUMENTS) {
                return abstractCommand.execute(parameters[POSITION_ARGUMENT_OF_COMMAND_IN_INPUT], request);
            } else {
                return abstractCommand.execute(DEFAULT_ARGUMENT_OF_COMMAND, request);
            }
        } catch (CommandNotFoundException e) {
            writer.println(e.getMessage());
            return new AbstractResponse(false, "что-то упало:(");
        }
    }
